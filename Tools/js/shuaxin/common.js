...
(function(exports) {
    ...
    exports.appendTestData = function(dom, count, isReset, index) {
        ...
        var template = '<li class="shuaxin_list"><a href="#"><img class="shuaxin_img" src="{{picUrl}}"></a><div class="pic-down">下载</div><div class="shuaxin_desc">日期：{{date}}</div></li>';
        ...
        $.get("http://gank.io/api/data/福利/10/"+page, {
			...
			if(data.error == true) {
				...
				mui.each(data.results, function(key, value) {
					
					dateStr = data.results[key].desc;
					html += exports.renderTemplate(template, {
//		                title: prevTitle + '【' + counterArr[counterIndex] + '】'+data.results[key].url,
						picUrl:data.results[key].url,
		                date: dateStr
		            });
